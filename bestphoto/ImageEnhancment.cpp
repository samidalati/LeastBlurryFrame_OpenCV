...
//1st: count the number of edges, the highest edge number is the clear one.
...
int main(int argc, char** argv)
{
    ...
    for(int fr_num=1;fr_num<numOfFrame;fr_num++)
    {   
        char buffer [50];
        ...
        cvtColor(frame, grayTemp, CV_BGR2GRAY);
        Canny(grayTemp, grayTemp, 100, 200, 3);
        ...
        cout<<"frame "<<fr_num<<" # of contours: "<<contours.size()<<endl;
        if(contours.size() > lastContourSize)
            bestFrameNum = fr_num;
        /// Draw contours
